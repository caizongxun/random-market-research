...
對指定時間段做 Walk-Forward Grid Search：
  - 每隔 --step 根取一個預測起點
  - 對每個起點用 grid search 找讓 mae_pct 最小的 (drift_scale, momentum_boost, drift_decay) 組合
  - 同時記錄前500根的特徵向量
  - 把 (特徵, 最佳參數) 寫進 CSV
...
def main():
    ...
    for i, train_end_idx in enumerate(candidates):
        ...
        print(f"[{i+1}/{len(candidates)}] 起點 {date_str} (idx={train_end_idx})  ", end="", flush=True)
        ...
        # 3. grid search
        best_mae    = float("inf")
        best_params = {"drift_scale": 1.0, "momentum_boost": 1.3, "drift_decay": 0.07}

        for ds, mb, dd in grid:
            mae = run_sim_and_mae(
                df, train_end_idx, theta, args,
                drift_scale=ds, momentum_boost=mb, drift_decay=dd,
                vol_multiplier=vol_multiplier, n_paths=args.n_paths,
            )
            if mae is not None and mae < best_mae:
                best_mae    = mae
                best_params = {"drift_scale": ds, "momentum_boost": mb, "drift_decay": dd}

        print(f"best_mae={best_mae:.3f}%  ds={best_params['drift_scale']}  "
              f"mb={best_params['momentum_boost']}  dd={best_params['drift_decay']}")
